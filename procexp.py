...
try:
  ...
  _icon_theme = Gtk.IconTheme.get_default() # to get process icons
except:
  _icon_theme = None
...
def _get_cpu_plot_tooltip(history_fraction):
  """ returns info about top process on specific point (0-1.0) of main cpu plot
  """
...
def _apply_tree_altcolors():
  """ updates tree-rows alternating background colors
  """
...
class CPUPlotListener(QtCore.QObject):
  """ on cpu plot shows tooltip with info about process that used the most cpu
  """
  ...
  def eventFilter(self, object, event):
    if event.type() == QtCore.QEvent.ToolTip:
        ...
        ttt = _get_cpu_plot_tooltip(fraction)
      ...
      if self._show_tooltip:
        ...
        ttt = _get_cpu_plot_tooltip(fraction)
        globpos = self.__plot.mapToGlobal(event.pos())
...
class WindowListener(QtCore.QObject):
  """ * Updates alternatig background colors for tree rows.
    * Adds hotkeys to main window:
      * F4 - focus filter
      * F5 - toggle tree/flat view
      * Escape - reset filter
  """

  Key_Escape = 0x01000000 # reset filter
  ...
  def eventFilter(self, object, event):
    if event.type() == QtCore.QEvent.KeyPress:
      if event.key() == WindowListener.Key_F4: # filter
        event.accept()
        g_mainUi.filter.setFocus()

      elif event.key() == WindowListener.Key_F5: # tree
        event.accept()
        new_tree_type = 'flat' if is_tree_mode() else 'tree'
        set_tree_type(new_tree_type)

      elif event.key() == WindowListener.Key_Escape: # escape - clear filter
...
def get_icon(proc_cmd):
  """ tries to get icon of the process by its command line
  """
...
_cpuplot_listener = None
_window_listener = None
...
def on_filter(filter_str):
  """ filter change listener: filters tree rows
  """
...
def set_tree_type(_type):
  """ change tree between usual tree structure and flat list
  """
  global _tree_type
...
def _on_header_click(logical_ind):
  """ set tree to flat list if sorting by any column except proc-name
  """
  if logical_ind != 0:
    set_tree_type('flat')
...
def prepareUI(mainUi):
  ...
  global _cpuplot_listener, _window_listener

  _cpuplot_listener = CPUPlotListener(mainUi.qwtPlotOverallCpuHist)
  _window_listener = WindowListener(mainUi)
...
def take_all_children(item, removed_items):
  """ removes all tree-item children and puts in list `removed_items`
  """
  if item != None:
